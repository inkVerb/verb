...
# About message
aboutMsg="$(cat <<EOU
This is a family work routine that manages options.
EOU
)"

# If using isOption() Option Select
validOptions=(car door bicycle desk)
...
if [ -z "${SOb}" ]; then
  ...
  echo "${optName[b]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOc}" ]; then
  #SOc="DEFAULT" # Uncomment for optional default
  echo "${optName[c]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOd}" ]; then
  #SOd="DEFAULT" # Uncomment for optional default
  echo "${optName[d]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOe}" ]; then
  #SOe="DEFAULT" # Uncomment for optional default
  echo "${optName[e]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOf}" ]; then
  #SOf="DEFAULT" # Uncomment for optional default
  echo "${optName[f]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOg}" ]; then
  #SOg="DEFAULT" # Uncomment for optional default
  echo "${optName[g]} must be set."; exit 0 # Uncomment if required
fi
if [ -z "${SOn}" ]; then
